...
_t_title "test-feat-close-api-gap"
...
# ---------------------------------------------------------------------------
_t_section "_shui_message_simple"
...
assert_eq "lines_before=2 produces 3 lines total" "3" "$_lines"

_out=$(_shui_message_simple "success" "msg")
_lines=$(printf '%s' "$_out" | grep -c '' || true)
assert_eq "default lines_before=0 produces 1 line" "1" "$_lines"

# ---------------------------------------------------------------------------
# 5. layout — divider flags, hr, center-text
# ---------------------------------------------------------------------------
_t_section "layout new features"
...
# ---------------------------------------------------------------------------
_t_section "progress custom chars"

_out=$(strip_ansi "$(_shui_progress 5 10 --width=10 --filled-char='#' --empty-char='.')")
assert_contains "--filled-char uses '#'" "#" "$_out"
assert_contains "--empty-char uses '.'" "." "$_out"
assert_not_contains "--filled-char='#' removes default '█'" "█" "$_out"

_out=$(strip_ansi "$(_shui_progress 5 10 --width=10)")
assert_contains "default filled char is '█'" "█" "$_out"
assert_contains "default empty char is '░'" "░" "$_out"

_out=$(strip_ansi "$(_shui_progress 3 4 --width=4)")
assert_contains "3/4 = 75%" "75%" "$_out"

# ---------------------------------------------------------------------------
# 8. pill aliases
# ---------------------------------------------------------------------------
_t_section "pill aliases"
...
# 13. link component
# ---------------------------------------------------------------------------
_t_section "_shui_hyperlink / _shui_print_link"
...
# 15. debug component
# ---------------------------------------------------------------------------
_t_section "_shui_debug"

unset SHUI_DEBUG
_out=$( _shui_debug "silent" 2>&1 )
assert_eq "silent without SHUI_DEBUG=true" "" "$_out"

_out=$( _shui_debug_vars SHUI_VERSION 2>&1 )
assert_eq "_shui_debug_vars silent without SHUI_DEBUG=true" "" "$_out"

_out=$( _shui_debug_command echo "silent" 2>&1 )
assert_eq "_shui_debug_command silent without SHUI_DEBUG=true" "" "$_out"

export SHUI_DEBUG=true

_out=$( _shui_debug "test message" 2>&1 )
assert_contains "emits message on stderr" "test message" "$(strip_ansi "$_out")"
assert_contains "includes [debug] prefix" "[debug]" "$(strip_ansi "$_out")"
...
assert_contains "_shui_debug_command runs the command" "ran" "$_out"
assert_contains "_shui_debug_command logs command to stderr" "printf" "$(strip_ansi "$_out")"
